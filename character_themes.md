# Adventure-Level Themes

> Each theme below is **adventure-level**.
> Each includes **4 Power Tags**, **1 Weakness**, a **Way of Magic**, and a **Narrative Constraint**.
> Balanced for one-shot play with younger players.

There are two kinds of themes: external-source (extrinsic) and internal-source (intrinsic). An intrinsic magic is something that is inherent in you, whereas an extrinsic one is a way you draw from or interact with your environment.

Extrinsic magics are more dependent on the environment, but they also give you the ability to externalize consequences. Whenever any magic happens, it has side-effects, and you can attempt to either contain them or redirect them. If you contain, it affects you. If you redirect, it affects the space around you. Intrinsic magics are always contained.

---

# Magic Systems Reference

Each theme uses one of these underlying systems:

## Building Status

A status accumulates on you or the environment. At threshold, something dramatic happens.

- **Intrinsic Building:** Status accumulates on you. At threshold (4), you gain dramatic power with constraints. At your limit (5), you transform—incredibly powerful but no longer fully yourself.
- **Extrinsic Building:** Status accumulates on the environment as story tags. At threshold (4), the environment transforms and becomes its own challenge.

**Standard Rule:** Count the power tags you use from this theme. Add a status at that tier (using 3 tags = add tier-3 status). You may resist this as a reaction if you have time and desire—but the magic still wants out.

## Opposing Status

Two poles exist in tension. Using one builds that status; the opposite cancels it out. Both poles have uses—the question is which dominates the scene.

**Standard Rule:** Count the power tags you use from this theme. Add a status at that tier to the environment. You may resist this as a reaction if you have time and desire. Statuses on the same axis cancel—remove tiers from the greater equal to the tier of the lesser.

## Pet/Harmony

You have a relationship with an entity (creature, spirit, or intelligent item). When you work together, you're stronger. When you fight its nature, it gets frustrated.

**Standard Rule:** Acting in harmony gives +1 Power. Acting against its nature adds 1 Strain. At Strain-4, it acts on its own.

## Immediate Price

When you use magic, you pay a price RIGHT NOW. Something transforms, vanishes, or breaks in a funny way.

**Standard Rule:** Small magic = small price. Big magic = big price. Prices are inconvenient or funny, never permanent.

## Immediate Demands

When you use magic, something demands payment RIGHT NOW. It wants a weird favor done immediately.

**Standard Rule:** Small help = small ask. Big help = big ask. Refusing builds Annoyed status on the helper.

## Binding Words

Your words have power and bind you the moment you speak them. But they also make you incredibly strong when you're keeping your word.

**Standard Rule:** Each promise adds 1 Bound. While keeping a promise, your magic is stronger. At Bound-4, your promises conflict and tangle you up.

## Fragmentation

Using magic creates copies or separated parts of yourself. These can act semi-independently.

**Standard Rule:** Each fragment is a story tag. At 3+ active fragments, they may act on their own or get targeted.

## Commitment

Magic only works when you commit to a position, person, or action. Your protection is absolute while committed, nothing while broken.

**Standard Rule:** Declare what you protect. While committed, your defense is extraordinary. Moving or changing breaks it completely.

---

## Ragebound Berserker

*Your fury is supernatural.*

**System:** Building Status (Intrinsic)

**Power Tags**

- Enraged Strength
- Ignore Pain
- Smash Through
- Intimidating Presence

**Weakness**

- Cannot Back Down

**Way of Magic – Rage**
When you use Ragebound power tags, count them and add **Rage** at that tier. Getting hurt or insulted also adds Rage-1. You may resist as a reaction if you have time.

- **Rage-1 to 3:** Rage adds to physical attacks and intimidation. Rage hinders careful, patient, or gentle actions.
- **Rage-4 (Threshold):** You MUST attack the biggest threat you can see. Your attacks are devastating but you can't hold back or be subtle.
- **Rage-5 (Limit):** You become the Rage. Unstoppable fury—you cannot distinguish friend from foe, only threats and obstacles.
- **Clear Rage:** Reduce by 1 when combat ends and you catch your breath, or when someone you trust calms you.

**Narrative Constraint**
Enemies will try to provoke you. Allies need to point you at the right targets.

---

## Chaos Channeler

*Magic misbehaves around you.*

**System:** Opposing Status (Chaos/Order)

**Power Tags**

- Reality Glitch
- Wild Surge
- Unlikely Success
- Spell Twist

**Weakness**

- Backfires

**Way of Magic – Chaos**
When you use Chaos Channeler power tags, count them and add **Chaos** at that tier to the environment. You may resist as a reaction if you have time.

- **Chaos status:** Helps any action that embraces randomness, improvisation, or the unexpected. Hinders any action that requires precision, planning, or reliability.
- **Chaos-4 (Threshold):** Reality gets weird. The GM gains story tags representing the strangeness: *gravity suggestions*, *doors go elsewhere*, *time hiccups*. These affect everyone.
- **Order opposes Chaos:** Deliberate, predictable actions (following a plan exactly, doing things by the book) add Order to the environment. Order and Chaos cancel each other.
- **Clear Chaos:** Chaos naturally reduces by 1 each scene, or when someone imposes structure and routine.

**Narrative Constraint**
The weirder things get, the more you thrive—but your allies might not.

---

## Cryomancer

*Cold is control.*

**System:** Opposing Status (Cold/Heat)

**Power Tags**

- Chill Out
- Ice Sculpting
- Numbing Chill
- Frozen Armor

**Weakness**

- Rigid and Brittle

**Way of Magic – Cold**
When you use Cryomancer power tags, count them and add **Cold** at that tier to the environment. You may resist as a reaction if you have time.

- **Cold status:** Helps actions involving control, barriers, slowing enemies, and precision. Hinders actions requiring speed, flexibility, or warmth.
- **Cold-4 (Threshold):** The environment freezes. The GM gains story tags: *treacherous ice*, *bitter silence*, *brittle surfaces*. Movement becomes difficult for everyone except you.
- **Heat opposes Cold:** Fire magic, warmth sources, or intense activity add Heat to the environment. Heat and Cold cancel each other.
- **Clear Cold:** Cold naturally reduces by 1 each scene, or faster when warmth is introduced.

**Narrative Constraint**
The battlefield grows silent and still. Warmth becomes precious.

---

## Elemental Savant

*One element, taken too far.*

**System:** Building Status (Extrinsic)

**Power Tags**

- Shape Element
- Elemental Blast
- Elemental Body
- Environmental Control

**Weakness**

- Elemental Mindset

**Way of Magic – Saturation**
When you use Elemental Savant power tags, count them and add **Saturation** at that tier to the environment (or contain it on yourself). You may resist as a reaction if you have time.

- **Saturation status:** The environment gains story tags matching your element. Saturation helps actions using your element; hinders actions opposing it.
- **Saturation-4 (Threshold):**
  - *Environment:* Your element dominates. The GM gains major story tags: *living stone walls*, *howling wind tunnels*, *churning water fills the room*. It becomes its own challenge.
  - *You (if contained):* You partially transform into your element. Add +2 Power to elemental actions, but you think like your element.
- **Saturation-5 (Your Limit):** Full elemental transformation. Incredibly powerful, but you ARE your element now.
- **Clear Saturation:** Reduce by 1 when the opposite element appears or after a scene without using your element.

**Narrative Constraint**
The environment increasingly acts like your element. Everything changes to match you.

---

## Name-Binder

*Names define reality.*

**System:** Building Status (Intrinsic)

**Power Tags**

- True Name
- Rename the World
- Words That Cut
- Naming Curse

**Weakness**

- Names Stick

**Way of Magic – Identity Strain**
When you use Name-Binder power tags, count them and add **Strain** at that tier. Bigger changes (a person's nature vs. an object's appearance) add +1 tier. You may resist as a reaction if you have time.

- **Strain-1 to 3:** Your renamings work perfectly. Strain adds to actions where names and identity matter.
- **Strain-4 (Threshold):** Your own name becomes unstable. Others can rename YOU, and it works. You might answer to the wrong name or forget details about yourself.
- **Strain-5 (Limit):** You become what others name you. Your identity is completely fluid—powerful, but you are whoever the last person said you were.
- **Clear Strain:** Reduce by 1 when someone calls you by your true name and means it, or when you do something unmistakably YOU.

**Narrative Constraint**
People remember what they are *called*, not what they were. Names have power over everyone—including you.

---

## Songweaver

*Music shapes space and emotion.*

**System:** Opposing Status (Fast Tempo/Slow Tempo)

**Power Tags**

- Battle Rhythm
- Lullaby
- Crescendo
- Sonic Pulse

**Weakness**

- Needs to Be Heard

**Way of Magic – Tempo**
When you use Songweaver power tags, count them and add **Tempo** (Fast or Slow, your choice) at that tier to the environment. You may resist as a reaction if you have time.

- **Fast Tempo:** Helps quick actions, attacks, and bold moves. Hinders careful, calm, or patient actions.
- **Slow Tempo:** Helps calm actions, defense, and careful work. Hinders quick or aggressive actions.
- **Tempo-4 (Threshold):** The Tempo dominates the scene. At Fast-4, everyone acts in a frenzy—story tags like *everything accelerates*, *no time to think*. At Slow-4, time crawls—*movements drag*, *thoughts stretch*.
- **Fast and Slow oppose:** They cancel each other out. You (or others) can shift the tempo by playing the opposite.
- **Clear Tempo:** Tempo naturally reduces by 1 each round. Complete silence clears all Tempo.

**Narrative Constraint**
Silence, noise, and interruptions matter deeply. Your music affects everyone—friend and foe.

---

## Bearer of the Singing Sword

*The sword sings. You don't have to.*

**System:** Pet/Harmony

**Power Tags**

- Dissonant Howl
- Rhythmic Strike
- Echoing Parry
- Resonant Cleave

**Weakness**

- The Sword Has Opinions

**Way of Magic – Harmony**
The sword has emotions: it loves courage, hates cowardice, and has strong feelings about honor.

- **In Harmony:** When you act as the sword wishes (brave, honorable, direct), gain +1 Power on attacks and parries.
- **Dissonance:** When you act against its nature (retreating, deception, cruelty), gain 1 **Dissonance**.
- **Dissonance-4 (Threshold):** The sword acts on its own. It might sing when you need silence, refuse to cut someone it respects, or strike at targets you didn't choose.
- **Clear Dissonance:** Reduce by 1 when you let the sword do what it wants—one brave charge, one honorable choice, one moment of glory.

**Narrative Constraint**
The blade reacts to emotions. It will let everyone know what it thinks of them.

---

## Beast Controller

*You command a massive, monstrous beast.*

**System:** Pet/Harmony

**Power Tags**

- Monstrous Companion
- Shared Senses
- Coordinated Assault
- Devastating Charge

**Weakness**

- The Beast Is Willful

**Way of Magic – Instinct**
The beast has desires: hunt, protect territory, flee danger, eat.

- **With Instinct:** Commands aligned with instinct (attack prey, defend you, chase something) gain +1 Power.
- **Against Instinct:** Commands that fight instinct (hold still when threatened, ignore prey, approach fire) gain 1 **Strain**.
- **Strain-4 (Threshold):** The beast follows its instincts instead of your orders. It may flee, attack the wrong target, or refuse to move.
- **Clear Strain:** Reduce by 1 when you let the beast do ONE thing it really wants—chase, eat, scare someone, or take a nap.

**Narrative Constraint**
The beast remembers who hurt it and who fed it. It has feelings about everyone.

---

## Beast-Hearted

*You are not fully human anymore.*

**System:** Building Status (Intrinsic)

**Power Tags**

- Bestial Strength
- Heightened Senses
- Savage Leap
- Predator's Instinct

**Weakness**

- Easily Provoked

**Way of Magic – Ferality**
When you use Beast-Hearted power tags, count them and add **Ferality** at that tier. Hunting or fighting also adds Ferality-1. You may resist as a reaction if you have time.

- **Ferality-1 to 3:** Ferality adds to physical actions, senses, and intimidation. Ferality hinders patience, etiquette, and fine manipulation.
- **Ferality-4 (Threshold):** You act on instinct. Incredibly powerful—but you think like an animal. Chase what runs, fight what challenges you, protect your pack.
- **Ferality-5 (Limit):** Full beast. You ARE the animal now. Immense power, but human concerns and plans are gone.
- **Clear Ferality:** Reduce by 1 when you do something civilized (share food politely, have a calm conversation, wait patiently).

**Narrative Constraint**
Patience is hard. Doors and etiquette are harder. But your senses and strength are incredible.

---

## Spirit Speaker

*Spirits always want something.*

**System:** Immediate Demands

**Power Tags**

- Call Spirits
- Spirit Sight
- Bargain with the Unseen
- Channel Otherworldly Power

**Weakness**

- Haunted

**Way of Magic – Spirit Bargains**
When you use spirit magic, a spirit helps—but demands something RIGHT NOW.

- **Small Help (1 power tag):** Small ask—"Tell me a secret," "Break something small," "Say my name three times."
- **Big Help (2+ power tags):** Big ask—"Give away something you value," "Do something embarrassing," "Let me borrow your voice for a minute."
- **Refuse Payment:** If you can't or won't pay, the spirit gets **Annoyed-1**. At Annoyed-4, spirits around you are SO distracting—making noise, hiding your stuff, giving bad advice loudly.
- **Clear Annoyed:** Reduce by 2 when you finally give a spirit what it wants.

**Narrative Constraint**
Spirits are always watching. They have opinions about everything and everyone.

---

## Doppelganger

*You are many people.*

**System:** Building Status (Intrinsic)

**Power Tags**

- Perfect Mimicry
- Borrowed Faces
- Social Infiltration
- Voice and Mannerisms

**Weakness**

- Identity Drift

**Way of Magic – Confusion**
When you use Doppelganger power tags, count them and add **Confusion** at that tier. You may resist as a reaction if you have time.

- **Confusion-1 to 3:** Your disguises are perfect. Confusion adds to any action where being someone else helps.
- **Confusion-4 (Threshold):** You slip between faces without meaning to. The GM can have you accidentally shift at awkward moments, or use the wrong voice.
- **Confusion-5 (Limit):** You forget which face is real. You might BE the person you're imitating—their memories, their loyalties, their fears.
- **Clear Confusion:** Reduce by 1 when you spend time as your true self, doing something only the real you would do.

**Narrative Constraint**
People react to who they *think* you are. Keep track of which face you're wearing.

---

## Merchant of Impossible Goods

*Everything has a price.*

**System:** Immediate Price

**Power Tags**

- Produce Anything
- Make a Deal
- Twist the Fine Print
- Unexpected Inventory

**Weakness**

- Owed Favors

**Way of Magic – Prices**
When you produce something impossible, the universe takes payment NOW.

- **Small Item (1 power tag):** Small price—your shoelaces untie, your hair changes color, your voice squeaks for a minute.
- **Big Item (2+ power tags):** Big price—your sword turns to flowers, your map goes blank, your shoes swap left and right, gravity works funny for you.
- **Prices Are Funny:** The universe thinks irony is hilarious. You can try to aim the price at something you don't care about, but the universe has preferences.
- Prices fade at the end of the scene. The embarrassment lasts forever.

**Narrative Constraint**
The universe always gets paid. It prefers comedy.

---

## Mirror Adept

*You are never where you seem.*

**System:** Fragmentation

**Power Tags**

- Create Reflections
- Swap Places
- Reflect Attacks
- Mirror Step

**Weakness**

- Fragile Anchors

**Way of Magic – Reflections**
Your tricks create **Reflections**—semi-real copies that exist in mirrors, water, or polished surfaces.

- **Active Reflections:** Each reflection is a story tag. You can see through them, swap places with them, or have them take simple actions.
- **Reflections Can Be Destroyed:** Breaking the surface they inhabit shatters them. Enemies learn to break mirrors.
- **3+ Reflections (Threshold):** Things get confused. You might swap when you didn't mean to, reflections might act on their own, or you might forget which one is the real you.
- **Clear:** Dismiss reflections to reduce your count. Consolidate back into one true self.

**Narrative Constraint**
More reflections means more options—but also more targets and more confusion.

---

## Shadowbound

*You are halfway gone.*

**System:** Opposing Status (Fading/Presence)

**Power Tags**

- Melt into Darkness
- Shadow Strike
- Umbral Escape
- Cloak of Gloom

**Weakness**

- Light Burns

**Way of Magic – Fading**
When you use Shadowbound power tags, count them and add **Fading** at that tier to yourself. You may resist as a reaction if you have time.

- **Fading status:** Helps stealth, escape, and slipping through. Hinders being noticed, getting help, and affecting the physical world.
- **Fading-4 (Threshold):** You're barely there. Incredibly hard to hurt—but allies' aid barely reaches you. People forget you're there mid-conversation. Story tags: *half-real*, *slipping away*.
- **Fading-5 (Limit):** You become shadow. Immensely powerful in darkness, but sunlight is agony and you may not be able to return.
- **Presence opposes Fading:** Bright light, physical contact, or someone calling your name with feeling adds Presence. Presence and Fading cancel each other.
- **Clear Fading:** Reduce by 1 when you step into bright light or when someone grabs you and says your name like they mean it.

**Narrative Constraint**
Shadows are your home. But the more you fade, the less real you become. Your allies can anchor you.

---

## Blood of Giants

*Ancient power lives in your body.*

**System:** Building Status (Extrinsic)

**Power Tags**

- Titanic Strength
- Earth-Shaking Blows
- Unstoppable Advance
- Towering Presence

**Weakness**

- Big Target

**Way of Magic – Shockwaves**
When you use Blood of Giants power tags, count them and add **Shockwaves** at that tier to the environment. You may resist as a reaction if you have time.

- **Shockwaves status:** The ground trembles. Shockwaves add to intimidation and raw power actions. Shockwaves hinder balance, precision, and delicate work—for everyone.
- **Shockwaves-4 (Threshold):** The environment becomes unstable. The GM gains story tags: *cracking floor*, *falling debris*, *nothing stands steady*. Everyone has trouble standing—except you.
- **Clear Shockwaves:** Reduce by 1 each round when you're not stomping around. Moving to a new area resets to 0.

**Narrative Constraint**
Subtlety is difficult. Ceilings have load limits. Everything notices you.

---

## Iron Defender

*You decide where the line is.*

**System:** Commitment

**Power Tags**

- Shield Wall
- Hold the Line
- Bodyguard
- Brace for Impact

**Weakness**

- Slow to Reposition

**Way of Magic – Commitment**
Your defense only works where you **commit**. Half-measures provide nothing.

- **Declare Your Commitment:** At the start of each scene (or when you choose), declare what you are protecting: a person, a doorway, an area, an object.
- **While Committed:** Your defensive abilities are extraordinary. You can stop attacks that would bypass normal defenses. You can hold against impossible odds.
- **Breaking Commitment:** Moving away from your commitment, or trying to protect something else, breaks the protection COMPLETELY. You must declare again.
- **The Hard Choice:** You cannot protect two things at once. Enemies WILL make you choose.

**Narrative Constraint**
You cannot save everyone at once. But what you commit to, nothing can touch.

---

## Pyromancer

*Fire always wants more.*

**System:** Opposing Status (Heat/Cold)

**Power Tags**

- Inferno Burst
- Living Flame
- Fire Immunity
- Explosive Spread

**Weakness**

- Everything Burns

**Way of Magic – Heat**
When you use Pyromancer power tags, count them and add **Heat** at that tier to the environment. You may resist as a reaction if you have time.

- **Heat status:** Fire spreads and builds. Heat helps fire attacks, intimidation, and destruction. Heat hinders precision, patience, and anything that doesn't like being on fire.
- **Heat-4 (Threshold):** Wildfire. The GM gains story tags: *everything burning*, *smoke everywhere*, *escape routes cut off*. The fire is its own challenge. You're immune; your allies aren't.
- **Cold opposes Heat:** Ice magic, water, or smothering adds Cold. Heat and Cold cancel each other.
- **Clear Heat:** Reduce by 1 when fires are put out or when you move to an unburned area.

**Narrative Constraint**
What you burn stays burned. Fire reshapes the battlefield permanently.

---

## Trick-Spark

*Magic loves a prank.*

**System:** Building Status (Extrinsic)

**Power Tags**

- Illusions
- Sudden Bursts
- Misdirection
- Spell Pranks

**Weakness**

- Unreliable

**Way of Magic – Mischief**
When you use Trick-Spark power tags, count them and add **Mischief** at that tier to the environment. You may resist as a reaction if you have time.

- **Mischief status:** The environment gains story tags representing pranks and coincidences: *slippery floors*, *things in wrong places*, *convenient distractions*. Mischief helps creative improvisation and trickery. Mischief hinders serious, careful, or dignified actions—for everyone.
- **Mischief-4 (Threshold):** Magic is pranking EVERYONE. The GM gains major story tags: *nothing is where it should be*, *gravity is a suggestion*, *cause and effect are negotiable*. The pranks aren't malicious, but they're relentless.
- **Clear Mischief:** Reduce by 1 when you let magic play harmlessly—small illusions, sparkles, harmless pranks with no stakes.

**Narrative Constraint**
Magic thinks everything is funny. You thrive in chaos; others might not.

---

## Fey-Touched Duelist

*Too fast, too clever, too bound.*

**System:** Binding Words

**Power Tags**

- Blink Step
- Enchanted Weapon
- Impossible Grace
- Duelist's Precision

**Weakness**

- Bound by Promises

**Way of Magic – Binding Words**
Every boast, promise, or accepted challenge binds you IMMEDIATELY.

- **Making Promises:** Say "I'll defeat you" and you literally cannot retreat from that fight. Say "I won't hurt you" and you physically can't attack that person.
- **Keeping Promises:** While you're doing what you promised, your magic is STRONGER. Add +1 Power when keeping your word.
- **Bound Status:** Each active promise adds 1 **Bound**. At Bound-4, your promises conflict and tangle. ("I'll protect you" + "I'll defeat him" = trouble when he grabs you as a shield.)
- **Ending Promises:** Promises end when fulfilled exactly or when the other person releases you from it.

**Narrative Constraint**
Every word matters. The clever duelist promises only what they intended to do anyway.

---

## Living Shadow

*You are a creature of darkness.*

**System:** Fragmentation

**Power Tags**

- Detach from Body
- Shadow Tendrils
- Darkvision Everywhere
- Slip Through Cracks

**Weakness**

- Tethered to Darkness

**Way of Magic – Separation**
You can separate your shadow from your body, operating as two entities.

- **While Separated:** Your shadow can scout, manipulate objects, or attack from darkness while your body acts elsewhere. Each counts as a story tag.
- **Drift:** Each time you separate or act far apart, gain 1 **Drift**.
- **Drift-4 (Threshold):** Body and shadow act at cross purposes. Your shadow might pursue its own goals, hide information from you, or refuse to rejoin.
- **Clear Drift:** Reduce by 2 when you reunite in darkness. Reduce by 1 when you feed your shadow something it wants (scaring someone, learning a secret, swallowing a light source).

**Narrative Constraint**
Bright light forces your shadow back and causes pain. Your shadow has been learning things without you.
